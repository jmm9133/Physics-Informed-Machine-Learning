...
def golub_kahan_svd(B, max_iterations=1000, epsilon=1e-10):
    """
...
def golub_reinsch_svd(A, max_iterations=1000, epsilon=1e-10):
    """
    Compute the SVD of matrix A using the Golub-Reinsch algorithm.
    Returns U, S, V such that A = U * S * V^T.
    """
    U, B, V_T = bidiagonalize(A)
    U_svd, S, V_svd = golub_kahan_svd(B, max_iterations, epsilon)
    U_final = U @ U_svd
    V_final = V_T.T @ V_svd
...
print(X.shape)
X = mnist.data.to_numpy()  # Converts DataFrame to NumPy array
print(X.shape)
