...
def train_model(model, X_train, y_train, X_val, y_val, epochs=15000):
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, X_train.size(0), batch_size):
            ...
            optimizer.zero_grad()
            pred_y = model(batch_X, t_span)  # pred_y has predictions for each time step
            
            # Compute loss across time steps
            time_step_losses = []
            for t in range(pred_y.size(0)):
                time_step_loss = criterion(pred_y[t], batch_y)
                time_step_losses.append(time_step_loss)
            
            # Aggregate time-step losses (e.g., by averaging)
        ...
        # Validation loss (also across all time steps)
        model.eval()
        with torch.no_grad():
            val_pred = model(X_val, t_span)
            val_time_step_losses = [criterion(val_pred[t], y_val) for t in range(val_pred.size(0))]
            val_loss = torch.mean(torch.stack(val_time_step_losses))
