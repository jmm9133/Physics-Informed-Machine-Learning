...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 17 22:07:54 2024
...
'''def forward_auto_diff(Y_hat, Y, memory, params_values, nn_architecture, X):
    memory = {}
    A_curr = X
    memory["A0"] = X
    ...
    # Initial gradient
    dA_prev = (Y_hat - Y) / X.shape[0] if not np.isscalar(Y) else (Y_hat - Y)
    ...
    for layer_idx, layer in enumerate(nn_architecture):
        ...
        dW_curr = np.dot(A_curr.T, dA_prev)
        db_curr = np.sum(A_curr, axis=0)
        db_curr = db_curr[:, np.newaxis]
        # Store gradients
        grads_values["dW" + str(layer_idx_curr)] = dW_curr
        grads_values["db" + str(layer_idx_curr)] = db_curr
        ...
        dA_prev = np.dot(dA_prev, W) * dZ
        
        # Store layer activations
        memory["A" + str(layer_idx_curr)] = A_curr
        memory["Z" + str(layer_idx_curr)] = Z_curr
    
    return A_curr, grads_values'''
def forward_auto_diff(Y_hat, Y, params_values, nn_architecture, X):
    """
    Implements forward mode automatic differentiation.

    Args:
    - Y_hat: Predicted output (to calculate loss gradient with Y).
    - Y: Ground truth labels.
    - params_values: Dictionary of model parameters (weights, biases).
    - nn_architecture: List of dictionaries specifying the architecture.
    - X: Input features.

    Returns:
    - A_curr: Final output (Y_hat).
    - memory: Memory of activations and their derivatives.
    """
    memory = {}
    A_curr = X  # Primary variable
    dA_curr = np.ones_like(X)  # Derivative of the input (dual variable)
    memory["A0"] = X
    memory["dA0"] = dA_curr
...
def update_forward(params_values, memory, nn_architecture, learning_rate):
    """
    Updates weights and biases based on gradients from forward mode AD.

    Args:
    - params_values: Dictionary of model parameters (weights, biases).
    - memory: Memory containing activations and their derivatives.
    - nn_architecture: List of dictionaries specifying the architecture.
    - learning_rate: Step size for gradient descent.

    Returns:
    - Updated params_values dictionary.
    """
    for layer_idx, layer in enumerate(nn_architecture):
        layer_idx_curr = layer_idx + 1
